...
*Graphs with 1,000, 10,000, and 100,000 samples looks approximately normal(the 100 sample size is nearing normal but still has some skew, especially compared to the other graphs that almost completely stay within the confidence interval bounds). This is consistent with what I expected because as the sample number increases, the more precise the confidence intervals will be and the more normal the sample will look, following the central limit theorem. *
...
# 3.3 
# convert food to be single vector of the food column
food <- food$Food
food
...
length(unique(food)) #155

# curry
food <- gsub(".*curry.*|.*curries.*", "curry", food)
length(unique(food)) #152

# lasagna
food <- gsub("lasagne", "lasagna", food)
length(unique(food)) #151
...
length(unique(food)) #146

# cheese
food <- gsub(".*cheese.*", "cheese", food)
length(unique(food)) #143
...
# I noticed in the table there was a "my favorite kinf of ..." I went to my backup and found pastries had been erranously removed behind food so I reinserted that. I also confirmed in the backup that freeze dried was the full entry and it was 
food <- gsub("my favorite kind of", "pastries", food)

# i am making a vegetable category 
food <- gsub("corn|sundried tomatoes|sundried tomatoes|sweet potato|fried okra|artichoke", "vegetables", food)

# i am adding to the fruit category 
food <- gsub("nectarine|strawberries|bananas|mango|fruits|dried fruits|platanos|dried mangos", "fruit", food)

# i am making a breakfast category 
food <- gsub("crepes|pastries|cinnamon roll|bagel|multigrain pancake", "breakfast foods", food) # must change pancakes before desserts

# i am making a deserts category 
food <- gsub("cookies|cake|donuts|dessert|desserts", "desserts", food)

# i am making a chicken dishes category 
food <- gsub("chicken|chicken wings|fried chicken|butter chicken|chicken malai kabab|chicken tikka masala|chicken parmesan|chicken tikka|chicken tenders|gizzard chicken|grilled chicken breast|jollof rice with chicken dishes", "chicken dishes", food)

# i am making a snacks category 
food <- gsub("cheez its|sweet chili doritos", "snacks", food)

# i am making a seafood category 
food <- gsub("salmon|blue point oyseter|ceviche|fish tacos|fried fish|fish|poke bowl", "seafood", food)

# other country cuisines
food <- gsub("brazilian|albanian|ethiopian|vietnamese|indonesian|lebanese|persian|puerto rican", "other national cuisines", food)

# other country dishes
food <- gsub("kelewele its an african dish|palak paneer|shahi paneer|tagine|farofa|jollof rice|hibachi|swedish meatballs|roast dinner", "other national dishes", food)

# specific pizza
food <- gsub("good pizza|lahmacun turkish pizza", "pizza", food) # adding these to pizza

# specific breads
food <- gsub("arepa|baguettes", "bread", food) # adding these to bread

# specific meats
food <- gsub("ribs|ham|hot dog", "meats", food) # adding these to bread

# wrapped things 
food <- gsub("empanadas|enchiladas|dumplings|flautas|potstickers|gyros", "wrapped delicacies", food) # once I got down to the bottom, there were many wrapped things

# drop two delicious only entries 
food <- gsub("delicious", "", food)
food <- food[food != ""] # drop the blank value made from replacing delicious
length(unique(food))
...
*There was at least one food that would have been erraneously categorized I was suprised at how hard it was to decide how to categorize foods*
