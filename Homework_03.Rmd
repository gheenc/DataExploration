...
This homework assignment contains 3 problems. Edit this `.Rmd` file and
insert your responses under the appropriate problems. Submit the knitted
output of this file as a .pdf file.

Your document should look nicely formatted, with R code, plots, and text
descriptions nicely integrated.

**CRITICAL 1 - for every included chunk below, remove `eval = F` before
knitting!!!!**

**CRITICAL 2 - Please submit knitted version of this assignment as a PDF
file on GRADESCOPE You can knit to a DOC and then convert to PDF in Word
(choose SAVE AS), or you can knit directly to PDF if that works for
you.**

**DELETE LINES 21 THROUGH 30 (INCLUSIVE) BEFORE KNITTING AND
SUBMITTING!**
...
*Graphs with 1,000, 10,000, and 100,000 samples looks approximately
normal(the 100 sample size is nearing normal but still has some skew,
especially compared to the other graphs that almost completely stay
within the confidence interval bounds). This is consistent with what I
expected because as the sample number increases, the more precise the
confidence intervals will be and the more normal the sample will look,
...
*There are casing inconsistencies (burritos vs Burritos). Some people
have additionally commentary (Thai food vs Thai food is my favorite).
There are punctuation or random unknown character (*\n in ramen, !!
behind pizza). Misspellings could also be an issue. Some people did
cuisines and some people did specific foods (Mexican food vs burritos) -
this will require me to make a decision about how to handle and
categorize these.
...
# i am making a breakfast category - must come before desserts bc of pancakes and handles a misspelling
...
# i am making a desserts category 
food <- gsub("cookies|cake|donuts|dessert|desserts", "desserts", food)

# other country dishes - must come before chicken dishes 
...
# specific meats
food <- gsub("ribs|ham", "meat", food) # adding these to bread

# wrapped things 
food <- gsub("empanadas|enchiladas|dumplings|flautas|potstickers|gyros|postickers", "wrapped delicacies", food) # once I got down to the bottom, there were many wrapped things

# replacing entries under 3 to be other
freq <- table(food)
food[food %in% names(freq[freq < 3])] <- "Other"

# how many entries have over3
#df_food[df_food$Freq >= 3, ]

length(unique(food))
sort(table(food), decreasing = TRUE, value = TRUE)# 37
...
# plot 
par(mar = c(10, 20, 10, 2), cex = 0.5) # change margin and text size 
