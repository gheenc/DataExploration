...
text(x = c(1:5), y = means - 10500, labels = paste(round(means, 2), "%"), cex = .8)
...
movie$log10pct <- movie$pctReturn + 0.05
...
boxplot(movie$log10pct ~ movie$Genre,
        ...
        las = 1,
        main = "Boxplot of Percent Returned by Genre")
means <- tapply(movie$log10pct, movie$Genre, mean)
points(means, col = "red3", pch = 18)
text(x = c(1:5), y = means - 10500, labels = paste(round(means, 2), "%"), cex = .8)
abline(h = 100, col = "blue3", lty = 3, lwd = 2)
```
...
*For this data, I actually prefer the stripplot because the box plot is so condensed, the IQR box and whiskers aren't adding any additional information,*
...
*No, I do not think the distribution of observations inside each Genre is normal.*
*No, I do not think the standard deviation is approximately the same in each Genre.*
...
*No, this ratio is well over 2, so we cannot assume variance is same across Genre.*
