...
for (i in 4:21){
  myTrib[i] <- sum(myTrib[i-3],myTrib[i-2],myTrib[i-1])
}
print(myTrib)
...
hist(compVals,
     main = "Frequency of NA in World Bank Columns",
     col = "steelblue4",
     xlab = "Number of NAs in Column")

# create percentage labels
compVals_percents <- round((compVals/nrow(wb)*100))
compVals_percents <- as.numeric(paste0(compVals_percents, "%"))
...
*Both histograms tell us that there are a lot of columns with missing values. The first histogram tells us the raw value (which is high) but does not provide any info on how many columns there are in the whole dataset, whereas the second histogram tells us that some of the columns have almost all of their columns empty.*
...
ans1 <- summary(smeans)
(round(as.numeric(ans1[4]),2 )) # NEED TO ROUND, currently a table and therefore not numeric
...
*Yes CLT does seem to be in effect because the data appears to be normally distributed around 4 which we know to be the true mean CHECK BC THE QUARTILE HAS SOME NON NORMALITIES. Yes, the mean of all the sample means is 4.006, which is very close to 4 which we know to be the true mean. Yes, the values are exactly the same. *
...
*Using the raw values not in log scale, the sand observations are very condensed and small. I prefer the log because you can appreciate the differences better and understand the full scope of data but the log does include negative values.Limestone has a higher concentration of iron compared to sandstone on average.*
...
test2 <- t.test(logIron ~ Rock, data=iron, conf.level = 0.99) # two different ways to do t test 
test2
```
...
n_length <- nrow(iron)
n_samples <- 10000
diffLogIron <- rep(NA, n_samples) # empty vector to hold the means 

for (i in 1:n_samples){
  s <- sample(iron$logIron, n_length, replace = TRUE) # samples from iron with replacement
  diffLogIron[i] <- mean(s) # saves the mean for that run
}
...
hist(diffLogIron,
     ...
     xlim = c(-1.5, 1.5)) # adjusted to ensure legend shows 

abline(v = confidence_interval, lwd = 3, col = 'red3') # CI of bootstraped 
abline(v = t.test(iron$logIron)$conf.int, lwd = 3, col = 'green4', lty = 4) # CI of t test
...
*From the histogram, we also see that the data is normally distributed but that 0 was included in the confidence interval for both the bootstrapped and normal t-test and therefore we cannot be confident the null hypothesis would be rejected. From the quantile plot, we can check and see that the sample means are normally distributed. *
...
# create empty variables needed
n_length <- nrow(iron) # reuse, technically dont need to redefine
n_samples <- 10000 # reuse, technically dont need to redefine
diffIron <- rep(NA, n_samples) # empty vector to hold the means 

for (i in 1:n_samples){
  s <- sample(iron$Iron, n_length, replace = TRUE) # samples from iron with replacement
  diffIron[i] <- mean(s) # saves the mean for that run
}
...
abline(v = t.test(iron$Iron)$conf.int, lwd = 3, col = 'green4', lty = 4) # CI of t test
legend("topright", c("t-Test CI", "Bootstrapped CI"), col = c("green4", "red3"), lwd = 3, lty = c(1,4))
...
*The histogram of the raw concentrations does not include 0 in the confidence interval so we would likely reject the null hypothesis in favor of the alternate that there is a difference in means. We can also see from the quartile plot that the data is less normally distributed. *
...
*To begin looking concentration-discharge relationship, we would need to create a new variable that takes the concentration and divides by discharge to create the concentration-discharge. This could be done using the mutate function, ensuring all NA's are dropped, the dividing concentration by discharge and saving it in the new created column with a meaningful name. It would be good to analyze these visually via a boxplot like the ion concentrations described above: subset the data to only be the state and the most recent concentration-discharge value for each ion, then do Concentration ~ Ion for each subsetting state. *
*I could analyze any significant differences between the groups utilizing a t test. Again, due to the multitude of variables, it would likely be advantageous to conduct the t test using a for loop. For comparing the most recent discharge concentration of the same mineral across state, I would write a for loop that would utilize a vector of a list of each state, a list of each ion, and a list of the ion concentration for that state. *
*If I wanted to be even more thorough, I could conduct permutation tests as well on all grouping listed above. This would involve. *
   *
